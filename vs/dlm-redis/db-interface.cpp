...
namespace dlm {
  DBInterface::DBInterface(const std::string &ip, uint16_t port) {
    //! Enable logging
  ...
  bool DBInterface::SetKeyValue(const std::string &key, uint64_t value, std::chrono::milliseconds expire) {
    ...
    auto tempKeyValueFuture = client.set_advanced(key, std::to_string(value), false, 0,
  ...
  bool DBInterface::UpdateExpire(const std::string &key, uint64_t value, std::chrono::milliseconds expire) {
    ...
    if (tempExistFlag == 0) {
      ...
      if (tempValue == std::to_string(value)) {
  ...
  bool DBInterface::DelKeyValue(const std::string &key, uint64_t value) {
    auto haveKeyFuture = client.exists(std::vector<std::string>{key});
    client.sync_commit();
    // show if the key exists
    uint64_t tempExistFlag = haveKeyFuture.get().as_integer();
  ...
  void DBInterface::DBInterfaceTest() {
